...
on the right hand side.  This is showing the possible paramters ![Parameters](Images/training_run/training_run_autest_runset_parms.jpg)

If you pressed Execute that will show the smae parmeters along with a way to provide input.  You will see that input parmeters are organized
...
* Highlight the "BASE_ALL_GEN_RF_SAMPLE001".  This will launch RF test.  The steps themselves may not make much sense at this point.
    * Note that RF emulator launched and it connected
    * See the "uc_test_exec_seqnum" argument that is generated for each run set.  That is an important concepot
    * Get familiar with how the execution is displayed
* Get familiar wiith following run sets.  See how arguments are laid out.  These three run sets form the core of the testing.  Understand how
